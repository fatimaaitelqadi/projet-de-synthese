    ...
    const fetchProduits = async () => {
      try {
        ...
        const filtered = productsData.filter(
          (p) => p.category === 'Heritage' || p.name.includes('Heritage')
        );
        ...
        console.error("Error fetching Heritage products:", err);
    ...
    <>
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div 
          ...
          style={{ 
            backgroundImage: 'url(https://images.unsplash.com/photo-1531995811006-35cb42e1a022?w=1200&h=800&fit=crop&crop=center)',
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Bijoux Heritage</h1>
            <p className="text-xl md:text-2xl mb-8">
              Des pièces inspirées de notre patrimoine culturel et historique
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="boucles-oreilles-container">
        ...
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun produit Heritage trouvé.</p>
          )}
        </div>
      </div>
    </>

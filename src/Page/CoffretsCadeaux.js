  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for gift boxes
        const filtered = productsData.filter(p => {
          console.log('Checking product:', p); // Debug log
          return p.categorie === 'coffret' || 
                 (p.nom && p.nom.toLowerCase().includes('coffret')) ||
                 (p.type && p.type.toLowerCase().includes('coffret'));
        });

        console.log('Filtered products:', filtered);

        if (filtered.length === 0) {
          console.warn('No gift boxes found after filtering');
        }
      ...
      <div className="boucles-oreilles-container">
        ...
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun coffret cadeau trouvé.</p>
          )}
        </div>
      </div>
    </>

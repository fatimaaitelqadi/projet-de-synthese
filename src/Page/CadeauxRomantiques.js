...
export default function CadeauxRomantiques() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for romantic gifts
        const filtered = productsData.filter(p => {
          console.log('Checking product:', p); // Debug log
          return p.categorie === 'romantique' || 
                 (p.nom && p.nom.toLowerCase().includes('romantique')) ||
                 (p.type && p.type.toLowerCase().includes('romantique'));
        });

        console.log('Filtered products:', filtered);

        if (filtered.length === 0) {
          console.warn('No romantic gifts found after filtering');
        }
      ...
      <div className="boucles-oreilles-container">
        ...
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun cadeau romantique trouvé.</p>
          )}
        </div>
      </div>
    </>

  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for Gold category
        const filtered = productsData.filter(
          (p) => p.category === 'Gold' || p.name.includes('Gold')
        );
      ...
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div className="absolute inset-0 w-full h-full">
          <video
            ...
            src="https://cdn.pixabay.com/vimeo/442893956/1080p.mp4" 
            autoPlay
            loop
            muted
            playsInline
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Gold Collection</h1>
            <p className="text-xl md:text-2xl mb-8">
              Timeless elegance crafted in pure gold for those special moments
            </p>
          </div>
        </div>
      </section>

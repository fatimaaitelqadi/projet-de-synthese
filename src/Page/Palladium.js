    ...
    const fetchProduits = async () => {
      try {
        ...
        const filtered = productsData.filter(
          (p) => p.category === 'Palladium' || p.name.includes('Palladium')
        );
        ...
        console.error("Error fetching Palladium products:", err);
  ...
  if (loading) {
    return (
      <div className="loading-container">
        ...
        <p>Loading Palladium products...</p>
      </div>
    );
  }
      ...
      <div className="boucles-oreilles-container">
        ...
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun produit en palladium trouvé.</p>
          )}
        </div>
      </div>
    </>

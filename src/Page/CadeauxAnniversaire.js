...
export default function CadeauxAnniversaire() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for anniversary gifts
        const filtered = productsData.filter(p => {
          console.log('Checking product:', p); // Debug log
          return p.categorie === 'anniversaire' || 
                 (p.nom && p.nom.toLowerCase().includes('anniversaire')) ||
                 (p.type && p.type.toLowerCase().includes('anniversaire'));
        });

        console.log('Filtered products:', filtered);

        if (filtered.length === 0) {
          console.warn('No anniversary gifts found after filtering');
        }
      ...
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Cadeaux d'Anniversaire</h1>
            <p className="text-xl md:text-2xl mb-8">
              Des cadeaux exceptionnels pour célébrer les moments spéciaux
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="boucles-oreilles-container">
        ...
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun cadeau d'anniversaire trouvé.</p>
          )}
        </div>
      </div>
    </>

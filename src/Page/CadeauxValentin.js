...
export default function CadeauxValentin() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for Valentine's gifts
        const filtered = productsData.filter(p => {
          console.log('Checking product:', p); // Debug log
          return p.categorie === 'valentin' || 
                 (p.nom && p.nom.toLowerCase().includes('valentin')) ||
                 (p.type && p.type.toLowerCase().includes('valentin'));
        });

        console.log('Filtered products:', filtered);

        if (filtered.length === 0) {
          console.warn('No Valentine gifts found after filtering');
        }
  ...
  if (loading) {
    return (
      <div className="loading-container">
        ...
        <p>Chargement des cadeaux de Saint-Valentin...</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="boucles-oreilles-container">
        <h2>Cadeaux de Saint-Valentin</h2>
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun cadeau de Saint-Valentin trouvé.</p>
          )}
        </div>
      </div>
    </>

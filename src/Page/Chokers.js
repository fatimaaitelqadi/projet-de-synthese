...
export default function Chokers() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for Chokers category
        const filtered = productsData.filter(
          (p) => p.category === 'Chokers' || p.name.includes('Chokers')
        );
        ...
        console.error("Error fetching Chokers products:", err);
      ...
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Chokers Collection</h1>
            <p className="text-xl md:text-2xl mb-8">
              Élégance et style avec notre collection de chokers raffinés
            </p>
          </div>
        </div>
      </section>

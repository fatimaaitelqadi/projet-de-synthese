...
export default function Coliers() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for Coliers category
        const filtered = productsData.filter(
          (p) => p.category === 'Coliers' || p.name.includes('Coliers')
        );
        ...
        console.error("Error fetching Coliers products:", err);
      ...
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Colliers Collection</h1>
            <p className="text-xl md:text-2xl mb-8">
              Des colliers élégants pour sublimer votre style au quotidien
            </p>
          </div>
        </div>
      </section>

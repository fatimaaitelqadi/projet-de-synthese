...
export default function Chandeliers() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for Chandeliers category
        const filtered = productsData.filter(
          (p) => p.category === 'Chandeliers' || p.name.includes('Chandeliers')
        );
        ...
        console.error("Error fetching Chandeliers products:", err);
  ...
  if (loading) {
    return (
      <div className="loading-container">
        ...
        <p>Loading Chandeliers products...</p>
      </div>
    );
  }
      ...
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Chandeliers Collection</h1>
            <p className="text-xl md:text-2xl mb-8">
              Des boucles d'oreilles chandeliers somptueuses pour illuminer votre style
            </p>
          </div>
        </div>
      </section>

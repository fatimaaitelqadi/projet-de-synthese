...
// Import icons
import { FaPlus, FaStar, FaRegStar, FaStarHalfAlt } from 'react-icons/fa';
    ...
    const fetchProduits = async () => {
      try {
        ...
        const filtered = allProduits.filter(
          (p) => p.name === 'Anneaux' // Case-sensitive match
        );
      ...
      <section className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <div className="absolute inset-0 w-full h-full">
          <video
            ...
            src="https://cdn.pixabay.com/vimeo/76979868/1080p.mp4" 
            autoPlay
            loop
            muted
            playsInline
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center px-4">
          <div className="text-center text-white max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Elegant Ring Collection</h1>
            <p className="text-xl md:text-2xl mb-8">
              Handcrafted luxury rings that make a statement
            </p>
          </div>
        </div>
      </section>

...
export default function CadeauxMaman() {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        ...
        // Filter products for mother's gifts
        const filtered = productsData.filter(p => {
          console.log('Checking product:', p); // Debug log
          return p.categorie === 'maman' || 
                 (p.nom && p.nom.toLowerCase().includes('maman')) ||
                 (p.type && p.type.toLowerCase().includes('maman'));
        });

        console.log('Filtered products:', filtered);

        if (filtered.length === 0) {
          console.warn('No mother gifts found after filtering');
        }
      ...
      <div className="boucles-oreilles-container">
        ...
        <div className="product-grid">
          {produits.length > 0 ? (
            ...
            <p>Aucun cadeau pour maman trouvé.</p>
          )}
        </div>
      </div>
    </>
